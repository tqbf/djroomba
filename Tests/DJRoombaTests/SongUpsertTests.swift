...
struct SongUpsertTests {
    @Test func upsertDedupesOnMusicItemIdAndNamespace() async throws {
    ...
    @Test func sameMusicItemIdDifferentNamespaceAreDistinctSongs() async throws {
        let store = try TestSupport.freshStore()
    ...
    @Test func reimportKeepsPlaylistMembershipIntact() async throws {
        let store = try TestSupport.freshStore()
