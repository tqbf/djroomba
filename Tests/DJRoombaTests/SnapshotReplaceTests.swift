...
struct SnapshotReplaceTests {
  @Test
  func `snapshot replace swaps membership in order`() async throws {
  ...
  @Test
  func `snapshot replace does not touch app data or stats`() async throws {
    let store = try TestSupport.freshStore()
    try await store.upsertSongs([
  ...
  @Test
  func `snapshot replace is atomic on failure`() async throws {
    let store = try TestSupport.freshStore()
