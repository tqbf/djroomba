...
struct UIRefreshCorrectionTests {
    ...
    // MARK: - D3: summary equality drives the row refresh

    @Test func summaryEqualityReflectsTrackCount() {
        let a = PlaylistSummary(
            id: "p1", name: "Mine", trackCount: 0,
            isEditable: true, source: .appPlaylist, isFavorite: false
        )
        let b = PlaylistSummary(
            id: "p1", name: "Mine", trackCount: 2,
            isEditable: true, source: .appPlaylist, isFavorite: false
        )
    ...
    @Test func summaryEqualityReflectsName() {
        let a = PlaylistSummary(
            id: "p1", name: "Old", trackCount: 3,
            isEditable: true, source: .appPlaylist, isFavorite: false
        )
        let b = PlaylistSummary(
            id: "p1", name: "New", trackCount: 3,
            isEditable: true, source: .appPlaylist, isFavorite: false
        )
    ...
    @Test func summaryEqualityStillReflectsFavoriteAndIdentity() {
    ...
    // MARK: - D4: detail stats refresh on the play-recorded event

    @MainActor
    @Test func refreshStatsPicksUpAPlayRecordedAfterLoad() async throws {
        ...
        let summary = PlaylistSummary(
            id: pl.id, name: "Mine", trackCount: 2,
            isEditable: true, source: .appPlaylist, isFavorite: false
        )
    ...
    @MainActor
    @Test func reselectingACachedPlaylistRefreshesStaleStats() async throws {
        let store = try TestSupport.freshStore()
        try await store.upsertSongs([
            TestSupport.sampleSong(id: "a", musicItemID: "a", title: "A"),
        ])
        ...
        let service = PlaylistDetailService(store: store)
        let summary = PlaylistSummary(
            id: pl.id, name: "Mine", trackCount: 1,
            isEditable: true, source: .appPlaylist, isFavorite: false
        )
