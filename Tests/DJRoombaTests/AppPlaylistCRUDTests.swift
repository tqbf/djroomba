...
struct AppPlaylistCRUDTests {
    ...
    @Test func createAppendsAtEndOfSortOrder() async throws {
    ...
    @Test func renameUpdatesNameAndStampsUpdatedAt() async throws {
        let store = try TestSupport.freshStore()
        let created = try await store.createAppPlaylist(
            named: "Old", at: Date(timeIntervalSince1970: 1_000)
        )
        try await store.renameAppPlaylist(
            created.id, to: "New", at: Date(timeIntervalSince1970: 5_000)
        )
    ...
    @Test func deleteCascadesMembershipButNotSongsOrHistory() async throws {
        let store = try TestSupport.freshStore()
    ...
    @Test func addAppendsAndAllowsDuplicates() async throws {
        let store = try TestSupport.freshStore()
    ...
    @Test func removeByPositionRenumbersSurvivors() async throws {
        let store = try TestSupport.freshStore()
    ...
    @Test func setTracksReplacesWholeOrderedMembership() async throws {
        let store = try TestSupport.freshStore()
        try await seedSongs(store, ["a", "b", "c"])
        let pl = try await store.createAppPlaylist(named: "P")
    ...
    @Test func reorderAppPlaylistsPersistsNewSidebarOrder() async throws {
        let store = try TestSupport.freshStore()
    ...
    @Test func addAcrossChunkBoundaryKeepsOrder() async throws {
    ...
    @Test func appEditsNeverTouchImportedAppleSnapshot() async throws {
        ...
        let apple = ApplePlaylist(
            id: "apple-1", name: "Imported", artworkURL: nil,
            curator: nil, lastImportedAt: .now
        )
    ...
    @Test func songsWithStatsJoinsPlayCountForAppPlaylist() async throws {
        let store = try TestSupport.freshStore()
