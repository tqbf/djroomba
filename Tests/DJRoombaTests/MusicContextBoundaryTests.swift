...
struct MusicContextBoundaryTests {
    ...
    @Test func contextIsPlayingReflectsStatus() {
    ...
    @Test func contextEqualityCoversTheProjectedFields() {
        let a = MusicContext(
    ...
    @Test func contextCarriesOnlyStringsAndStatus() {
    ...
    @Test func commandSetIsClosedAndSendable() {
        ...
        for command in commands {
            switch command {
            case .playPlaylist, .playTrack, .pause, .resume, .skipNext, .skipPrevious:
                #expect(Bool(true))
            }
        }
    }
