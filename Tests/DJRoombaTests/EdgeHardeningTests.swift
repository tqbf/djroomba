...
struct EdgeHardeningTests {
    ...
    @MainActor
    @Test func rapidSwitchingLandsOnTheLastSelectionNotAStaleOne() async throws {
        ...
        func summary(_ p: AppPlaylist) -> PlaylistSummary {
            PlaylistSummary(
                id: p.id, name: p.name, trackCount: 1,
    ...
    @MainActor
    @Test func clearAfterSelectDropsAnInFlightLoad() async throws {
        let store = try TestSupport.freshStore()
        try await store.upsertSongs([
    ...
    @Test func oneUnplayableTrackDoesNotBreakTheRestOfTheQueue() {
        ...
        func row(_ pos: Int, _ id: String) -> TrackRow {
            TrackRow(
                song: Song(
                    id: id, musicItemID: id, idNamespace: .library,
                    title: "T\(pos)", artistName: "A", albumTitle: nil,
                    duration: nil, isExplicit: false, artworkURL: nil,
                    importedAt: .now
                ),
                position: pos
            )
