...
struct FavoritesRecentsTests {
  @Test
  func `favorite toggle round trips`() async throws {
  ...
  @Test
  func `favorite preserves source kind`() async throws {
    let store = try TestSupport.freshStore()
  ...
  @Test
  func `recents are most recent first and capped`() async throws {
    let store = try TestSupport.freshStore()
  ...
  @Test
  func `replaying A bumps timestamp without duplicating`() async throws {
    let store = try TestSupport.freshStore()
