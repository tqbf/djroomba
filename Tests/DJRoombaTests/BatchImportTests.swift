...
struct BatchImportTests {
    @Test func upsertPreservesStableSongIDOnReimport() async throws {
    ...
    @Test func upsertPreservesPlaylistFKAcrossReimport() async throws {
        let store = try TestSupport.freshStore()
        try await store.upsertSongs([
    ...
    @Test func batchedIDLookupIsCorrectAcrossAChunkBoundary() async throws {
        let store = try TestSupport.freshStore()
    ...
    @Test func chunkedMembershipInsertPreservesOrderForLargePlaylist() async throws {
        let store = try TestSupport.freshStore()
    ...
    @Test func emptyInputsAreNoOps() async throws {
        let store = try TestSupport.freshStore()
        try await store.upsertSongs([])
