...
public final class OpenAIChatModel: Model, @unchecked Sendable {
  ...
  ///   - reasoningEffort: optional `reasoning_effort` for gpt-5.x
  ///     reasoning models (DJROOMBA PATCH 3, 2026-05-30). One of
  ///     `"minimal"`, `"low"`, `"medium"`, `"high"`. `nil` omits the
  ///     field (server default = `"medium"`). Stored as a mutable
  ///     `var` so callers can override per-turn without
  ...
  /// DJROOMBA PATCH 3 (2026-05-30): OpenAI's `reasoning_effort` request
  /// field for gpt-5.x reasoning models. Valid wire values: `"minimal"`,
  /// `"low"`, `"medium"`, `"high"`. `nil` ⇒ omit the field, server
  /// default applies (`"medium"`).
  ///
  /// **Public, mutable `var`** by design: DJ Roomba's chat surface lets
  /// the user override the reasoning level per-turn (a segmented picker
  /// in the assistant pane chrome that snaps back to default after each
  /// send). The mutation lives on the existing single `OpenAIChatModel`
  /// instance so we don't re-instantiate (and re-bind through the
  /// `ContextWindow`'s `Model` reference) on every turn. The model is
  /// `@unchecked Sendable` and the field is read only from inside
  /// `postChatCompletion` on whatever actor `call(_:)` is invoked from
  /// — DJ Roomba's `GPTService` is `@MainActor`, so writes + reads are
  /// already serialised by the main actor. Consumers driving the model
  /// from multiple actors should serialise externally.
...
struct ChatCompletionRequest: Encodable, Sendable {
  ...
  /// DJROOMBA PATCH 3: omitted from the wire when `nil`; one of
  /// `"minimal"` / `"low"` / `"medium"` / `"high"` for gpt-5.x.
