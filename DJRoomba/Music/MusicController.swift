...
final class MusicController {
  ...
  /// Phase D (`plans/son-of-genre-map.md`) — "Listen to this" affordance.
  /// Reads `song_genre` rows for the given genre via the existing
  /// `songsWithStats(matchingGenre:)`, runs the pure
  /// `GenreTreeListenPicker` to choose a deterministic top-N with an
  /// artist-diversity cap, and starts playback on a **transient queue**
  /// — a synthetic `PlaylistDetail` with id `"genretree:<name>"` that
  /// routes through the existing `resolveAndPlay` path. `isGenre = true`
  /// keeps the synthetic id out of the recents bump (same posture as the
  /// existing genre-browsing path).
  ///
  /// Returns the picked tracks so the caller can surface them ("32
  /// tracks queued") or hand them to `saveListenAsAppPlaylist` for the
  /// "Save as Playlist" follow-up.
  @discardableResult
  func listenToGenre(_ genre: String) async -> [GenreTreeListenPicker.PickedSong] {
    guard let store else { return [] }
    do {
      let songs = try await store.songsWithStats(matchingGenre: genre)
      let candidates = songs.map { row in
        GenreTreeListenPicker.Candidate(
          songID: row.song.id,
          title: row.song.title,
          artistName: row.song.artistName,
          playCount: row.playCount,
          lastPlayedAt: row.lastPlayedAt,
        )
      }
      let picks = GenreTreeListenPicker.pick(from: candidates)
      guard !picks.isEmpty else { return [] }
      // Re-key picks against the original SongWithStat rows so we keep
      // each pick's full Song for the TrackRow (carries musicItemID +
      // namespace for the resolver).
      let songsByID = Dictionary(uniqueKeysWithValues: songs.map { ($0.song.id, $0) })
      let rows = picks.enumerated().compactMap { index, pick -> TrackRow? in
        guard let stat = songsByID[pick.songID] else { return nil }
        return TrackRow(songWithStat: stat, position: index + 1)
      }
      guard !rows.isEmpty else { return [] }
      var detail = PlaylistDetail(
        id: "genretree:\(genre)",
        name: "Genre Tree: \(genre)",
  ...
  /// Phase D follow-up — create a new app playlist named
  /// "Genre Tree: <name>" containing the same deterministic top-N pick
  /// the transient queue used. Selects the new playlist in the sidebar
  /// (same posture as the user's manual "+" affordance) so the user
  /// lands on it immediately.
  ///
  /// Returns the new playlist's id, or `nil` on failure / empty pick.
  @discardableResult
  func saveListenAsAppPlaylist(genre: String) async -> String? {
    ...
    do {
      ...
      let songIDs = picks.map(\.songID)
      let playlistName = "Genre Tree: \(genre)"
      guard let newID = await appPlaylistService.create(named: playlistName) else {
        return nil
      }
      rebuildDerivedSummaries()
      await appPlaylistService.addSongs(songIDs, to: newID)
      rebuildDerivedSummaries()
      detailService.invalidate(playlistID: newID)
