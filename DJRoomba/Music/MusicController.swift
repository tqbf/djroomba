...
final class MusicController {
    ...
    init() {
        ...
        self.library = LibraryReadService(store: safeStore)
        self.appPlaylistService = AppPlaylistService(store: safeStore)
        self.detailService = PlaylistDetailService(store: safeStore)
        self.importService = ImportService(store: safeStore)
        self.resolver = PlaybackResolver()
        if openedStore == nil {
            self.storeError = "The local library database could not be opened."
        }
    }
    ...
    // MARK: - Favorites & recents (SQLite-backed)
    ...
    // MARK: - Playback intents (resolve stored ids → MusicKit → play)
    ...
    private func resolveAndPlay(detail: PlaylistDetail, startRow: TrackRow?) async {
        ...
        if detail.isAppOwned {
            resolution = await resolver.resolveAppPlaylist(
                rows: detail.tracks,
                startAt: startRow
            )
        } else {
    ...
    // MARK: - App-playlist CRUD intents (SQLite-only; never written to Apple)
