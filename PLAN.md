...
| [plans/up-next-queue.md](plans/up-next-queue.md) | **Up Next queue** — ephemeral in-memory queue (`UpNextService`, `@MainActor @Observable`, lost on quit), sidebar landing row inside the Recently Played section (`upNextLandingID` sentinel, mirror of `RecentlyPlayedLandingRow`), `UpNextView` reusing `TrackTableView` with Play/Remove/Clear/Move-to-Top context menu. Queue dominates playback advance — `detectAndRecordAdvance` + `skipNext` consume head before falling through to playlist; click row #N consumes #1–#N. Four GPT tools (`up_next_add`, `up_next_count`, `up_next_get`, `up_next_remove`). Phase 5 ships an opt-in default-OFF auto-fill toggle that, when the queue depth crosses the low-water mark (`oldCount > 1 → newCount <= 1`, named `UpNextDrainDetector.refillThreshold`), mints a fresh assistant conversation seeded with a "pick eleven and call `up_next_add`" prompt (`UpNextDrainDetector.targetDepth = 11`). The 11-track batch + 1-remaining trigger lets the `gpt-5.4`-flex refill turn run under the cover of a still-playing song, so the user doesn't sit through dead air between batches. Five phases; one-song play uses the existing F1a `startResolvedQueue` path so no new playback infra. |
